...
class ServiceManager:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        if project_root is None:
            ...
            while current.parent != current:
                ...
                raise RuntimeError(
                    "Could not find project root with docker-compose.yml"
                )
    ...
    def _run_compose_command(
        self, command: list, capture_output: bool = True
    ...
    def _wait_for_service(self, service_name: str, timeout: int = 30) -> bool:
        ...
        logger.warning(
            f"⚠️  {service_name} service did not become ready within {timeout}s"
        )
        return False
