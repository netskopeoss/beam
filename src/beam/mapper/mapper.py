import re
import logging
from beam.mapper.data_sources import DataSource, APIDataSource
from beam.mapper.llm import LLMDataSource
from beam.mapper.gemini import query_gemini
from beam.mapper.agent_parser import query_agent_parser
...
DB_PATH = str(DATA_DIR / "mapper" / "user_agent_mapping.db")
...
class UserAgentMapper(DataSource):
    ...
    def search_datastore(self) -> None:
        ...
        if self.datastore.hits_found:
            self.logger.info(f"Found {len(self.datastore.hits)} user agents in the datastore.")
        ...
        if self.datastore.misses_found:
            ...
            return
        self.logger.info(f"A total of {len(self.misses)} misses exist after checking the datastore.")
    ...
    def search_api(self) -> None:
        ...
        if self.api.misses_found:
            ...
            self.logger.info("All user agents were mapped after checking the API data source.")
            return
        self.logger.info(f"A total of {len(self.misses)} misses exist after checking the API.")
    ...
    def save_results(self) -> None:
        ...
        """
        if self.hits_found:
            self.logger.info(f"A total of {len(self.hits)} user agents were found. Writing those to the db.")
...
def query_user_agent_mapper(
    ...
    user_agent_input = [ user_agent for user_agent in user_agents\
            if not re.match("^$", user_agent) ]

    logger.info(f"A total of {len(user_agent_input)} user agents were provided to the mapper.")
