...
def temp_workspace():
    ...
    workspace = {
        ...
        'models': {
            ...
            'combined_model': os.path.join(temp_dir, 'models', 'combined_app_model.pkl'),
        ...
        'files': {
            ...
            'enriched_events': os.path.join(temp_dir, 'data', 'enriched_events', 'test_traffic.json'),
            'app_features': os.path.join(temp_dir, 'data', 'app_summaries', 'test_app_features.json'),
            'custom_model': os.path.join(temp_dir, 'models', 'custom_models', 'TestApp_model.pkl'),
        }
    }
            ...
            "entries": [
                {
                    ...
                    "request": {
                        ...
                        "headers": [
                            {"name": "User-Agent", "value": "TestApp/1.0.0 (Windows NT 10.0; Win64; x64)"},
...
class TestEndToEndTrainingWorkflow:
    ...
    def test_process_training_data_complete_workflow(
        ...
        # Setup mocks
        mock_parse_input.return_value = ("test_traffic", temp_workspace['files']['enriched_events'])
        ...
        # Create pre-trained model file to trigger merge
        pretrained_models = [{"key": "ExistingApp", "estimator": "mock_estimator"}]
        with open(temp_workspace['models']['pretrained_model'], 'wb') as f:
            pickle.dump(pretrained_models, f)
        
        # Execute training workflow
        with patch('beam.run.Path.exists', return_value=True):
        ...
        mock_trainer.merge_models.assert_called_once()
    ...
    def test_process_training_data_no_pretrained_model(
        ...
        """Test training workflow when no pre-trained model exists"""
        # Setup mocks
        mock_parse_input.return_value = ("test_traffic", temp_workspace['files']['enriched_events'])
        mock_enrich_events.return_value = temp_workspace['files']['enriched_events']
        mock_extract_features.return_value = temp_workspace['files']['app_features']
        ...
        # Execute training workflow without pre-trained model
        with patch('beam.run.Path.exists', return_value=False):
            run.process_training_data(
                input_file_path=temp_workspace['files']['har_input'],
                app_name="TestApp",
                custom_model_path=temp_workspace['files']['custom_model'],
    ...
    def test_full_model_training_pipeline(self, temp_workspace, mock_app_features_sufficient):
    ...
    def test_model_merging_integration(self, temp_workspace, mock_app_features_sufficient):
...
class TestCommandLineIntegration:
    ...
    def test_command_line_training_invocation(self, mock_parse_args, mock_process_training):
    ...
    def test_command_line_training_parameters(
        ...
        mock_extract_features.return_value = "/path/to/features.json"
        mock_train_custom.return_value = None
        
        mock_logger = MagicMock()
        
        # Execute with mocked file parsing
        with patch('beam.run.parse_input_file', return_value=("test", "/enriched.json")):
            with patch('beam.run.enrich_events', return_value="/enriched.json"):
                with patch('beam.run.glob.glob', return_value=['/custom/input/test.har']):
                    with patch('beam.run.Path.exists', return_value=False):
                        run.run(logger=mock_logger)
        
        # Verify correct parameters were used
        mock_train_custom.assert_called()
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_training_with_permission_errors(self, mock_makedirs, temp_workspace):
        ...
        with patch('beam.run.parse_input_file', return_value=("test", "/enriched.json")):
            with patch('beam.run.enrich_events', return_value="/enriched.json"):
                with patch('beam.run.extract_app_features', return_value="/features.json"):
                    with patch('beam.run.train_custom_app_model', side_effect=PermissionError()):
                        try:
                            run.process_training_data(
                                input_file_path=temp_workspace['files']['har_input'],
                                app_name="TestApp",
    ...
    def test_training_with_large_feature_set(self, temp_workspace):
        ...
        for i in range(5):  # Create multiple feature sets
            features = {
                ...
                "req_content_types": ["application/json", "text/plain", "multipart/form-data"],
                "resp_content_types": ["application/json", "text/html", "image/png"]
            }
            # Add all numeric fields with varied values
            for field in [
                "refered_traffic_pct", "referer_domain_cnt", "unique_actions",
                "http_status_cnt", "http_method_cnt", "req_content_type_cnt",
                "resp_content_type_cnt", "avg_time_interval_sec", "std_time_interval_sec",
                "median_time_interval_sec", "range_time_interval_sec", "range_timestamp",
                "max_time_taken_ms", "min_time_taken_ms", "sum_time_taken_ms",
                "std_time_taken_ms", "median_time_taken_ms", "range_time_taken_ms",
                "max_client_bytes", "min_client_bytes", "sum_client_bytes",
                "avg_client_bytes", "std_client_bytes", "median_client_bytes",
                "range_client_bytes", "max_server_bytes", "min_server_bytes",
                "sum_server_bytes", "avg_server_bytes", "std_server_bytes",
                "median_server_bytes", "range_server_bytes", "web_traffic_pct",
                "cloud_traffic_pct", "sequence_num_keys", "sequence_max_key_length",
                "sequence_min_key_length", "sequence_max_val", "sequence_min_val",
                "sequence_sum_val", "sequence_avg_val", "sequence_std_val",
                "sequence_median_val", "sequence_range_val"
            ]:
    ...
    def test_concurrent_model_training_simulation(self, temp_workspace):
        ...
        for app_name in apps:
            ...
            numeric_fields = [
                ...
                "median_time_interval_sec", "range_time_interval_sec", "range_timestamp",
                "max_time_taken_ms", "min_time_taken_ms", "sum_time_taken_ms",
                "avg_time_taken_ms", "std_time_taken_ms", "median_time_taken_ms",
                "range_time_taken_ms", "max_client_bytes", "min_client_bytes",
                "sum_client_bytes", "avg_client_bytes", "std_client_bytes",
                "median_client_bytes", "range_client_bytes", "max_server_bytes",
                "min_server_bytes", "sum_server_bytes", "avg_server_bytes",
                "std_server_bytes", "median_server_bytes", "range_server_bytes",
                "web_traffic_pct", "cloud_traffic_pct", "sequence_num_keys",
                "sequence_max_key_length", "sequence_min_key_length", "sequence_max_val",
                "sequence_min_val", "sequence_sum_val", "sequence_avg_val",
                "sequence_std_val", "sequence_median_val", "sequence_range_val"
            ]
            for field in numeric_fields:
                app_features[0][field] = 1.0
            
            features_file = os.path.join(temp_workspace['data']['app_summaries'], f"{app_name}_features.json")
            model_file = os.path.join(temp_workspace['models']['custom_models'], f"{app_name}_model.pkl")
