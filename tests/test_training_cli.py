...
class TestCommandLineArgumentParsing:
    ...
    def test_training_arguments_parsing(self):
        ...
        with patch('sys.argv', ['beam'] + test_args):
            ...
            parser.add_argument('--train', action='store_true', help='Train custom app model')
            parser.add_argument('--app_name', type=str, help='Name of the app to train for')
            ...
            parser.add_argument('--no-use_custom_models', dest='use_custom_models', action='store_false')
    ...
    def test_missing_required_training_arguments(self):
        """Test error handling for missing required training arguments"""
        # Training without app_name should be handled gracefully
        test_args = ['--train']
...
class TestTrainingWorkflowErrorHandling:
    ...
    def test_training_with_no_input_files(self, mock_glob, mock_process_training, temp_workspace):
    ...
    @patch('beam.run.process_training_data')
    @patch('beam.run.glob.glob')
    def test_training_with_invalid_input_directory(self, mock_glob, mock_process_training):
    ...
    @patch('beam.run.glob.glob')
    def test_training_with_parse_input_file_failure(self, mock_glob, mock_parse_input, temp_workspace):
    ...
    @patch('beam.run.parse_input_file')
    @patch('beam.run.glob.glob')
    def test_training_with_enrichment_failure(self, mock_glob, mock_parse_input, mock_enrich, temp_workspace):
    ...
    def test_training_with_model_training_failure(
        self, mock_glob, mock_parse_input, mock_enrich, mock_extract, mock_train, temp_workspace
    ):
...
class TestResourceManagementAndCleanup:
    ...
    @patch('beam.run.process_training_data')
    @patch('beam.run.glob.glob')
    def test_temporary_file_cleanup(self, mock_glob, mock_process_training, temp_workspace):
        ...
        def mock_process_with_temp_files(*args, **kwargs):
            ...
            temp_file = os.path.join(temp_workspace['input_dir'], 'temp_processing.json')
    ...
    def test_memory_usage_with_large_datasets(self, temp_workspace):
        ...
        large_features_file = os.path.join(temp_workspace['input_dir'], 'large_features.json')
    ...
    def test_resource_cleanup_on_training_failure(self, mock_trainer_class, temp_workspace):
        """Test resource cleanup when training fails"""
        # Mock trainer to fail during model creation
        mock_trainer = MagicMock()
        mock_trainer.add_app_model.side_effect = Exception("Training failed")
        ...
        # Attempt training that will fail
        try:
            with patch('beam.run.parse_input_file', return_value=("test", temp_enriched)):
                with patch('beam.run.enrich_events', return_value=temp_enriched):
                    with patch('beam.run.extract_app_features', return_value=temp_features):
                        run.process_training_data(
                            temp_workspace['har_file'],
                            "TestApp",
                            temp_workspace['model_output'],
                            logger
                        )
        ...
        # In a proper implementation, temp files should be cleaned up
        # even when training fails. For this test, we verify they exist
        # and then clean them up manually
        if os.path.exists(temp_enriched):
...
class TestConcurrencyAndRaceConditions:
    ...
    def test_directory_creation_race_condition(self, temp_workspace):
        ...
        nested_dir = os.path.join(temp_workspace['input_dir'], 'nested', 'deep', 'directory')
